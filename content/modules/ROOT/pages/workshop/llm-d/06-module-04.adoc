...
You've proven that 4 GPUs with naive scaling don't significantly reduce P95/P99 tail latency—your most frustrated users still experience slow responses. Now it's time to deploy llm-d with Prefix Cache Aware Routing to try and reduce tail latency through intelligent cache reuse.
...
Before, we were using the ServiceMonitor that came out of the box with KServe. Right now, we need to tweak some things to get the metrics to scrape properly for our user-workload-monitoring instance.

=== Create ServiceMonitor for llm-d router

Create a ServiceMonitor to scrape the Endpoint Picker:
...
=== Create ServiceMonitor for llm-d inference pool

Create a ServiceMonitor to scrape the vLLM instances:
