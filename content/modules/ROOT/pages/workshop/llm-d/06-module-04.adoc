          ...
          selector:
            matchExpressions:
              - key: kubernetes.io/metadata.name
                ...
                values:
                  - llm-d-project
      hostname: inference-gateway.apps.{openshift_cluster_ingress_domain}
      name: https
      port: 443
      protocol: HTTPS
      tls:
...
This can take 5-10 minutes as each instance pulls the model from the OCI registry.
...
----
# Get router pod name
ROUTER_POD=$(oc get pods -l app.kubernetes.io/component=llminferenceservice-router-scheduler -o jsonpath='{.items[0].metadata.name}')
...
oc get pods -l app.kubernetes.io/component=llminferenceservice-router-scheduler
oc describe pod -l app.kubernetes.io/component=llminferenceservice-router-scheduler

# Check inference pool pods
oc get pods -l kserve.io/component=workload
oc logs -l kserve.io/component=workload --tail=100

# Common issues:
# - GPU quota: Verify 4 GPUs available
# - Image pull: Check image access and pull secrets
# - Model download: Check OCI registry access
----
