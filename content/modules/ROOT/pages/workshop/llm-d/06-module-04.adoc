...
=== Configure storage initializer for HuggingFace

Create a ClusterStorageContainer to configure the storage-initializer with HuggingFace authentication:

[source,bash]
----
cat > hf-storage-container.yaml << 'EOF'
...
oc apply -f hf-storage-container.yaml
----

This ClusterStorageContainer ensures that any InferenceService using `hf://` URIs will have access to the HuggingFace token during model download.
...
* **`ClusterStorageContainer`**: Configures storage-initializer with HuggingFace token for model download
