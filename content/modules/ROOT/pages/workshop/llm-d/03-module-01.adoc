...
* Deploy Llama 3.1 8B using KServe and vLLM on OpenShift AI
* Explain the KV cache and why cache misses cause high P95/P99 latency
* Understand why naive horizontal scaling doesn't fix tail latency
* Describe how llm-d's prefix-cache-aware routing reduces P95/P99 latency
...
Three bottlenecks drive high P95/P99 latency in multi-GPU LLM deployments:

* **Prefill queuing**: Large prompts block smaller requests from starting (head-of-line blocking). A user with a small prompt who submits just after a large-prompt request can experience 3x higher TTFT simply due to queuing.
* **KV cache misses**: With round-robin routing, requests with shared prefixes land on different vLLM instances, forcing each to recompute the same system prompt or conversation history from scratch on every turn.
* **Unpredictable queue depth**: Random routing ignores how busy each instance actually is. P50 latency may be 200ms while P95 reaches 2000ms—a 10x gap.
...
[NOTE]
====
Keep the port-forward running—you'll need it in the next module to verify the metrics endpoint.
====
...
* Deployed Llama 3.1 8B Instruct on a single GPU using KServe and vLLM
* Verified the model is serving requests correctly

**Key takeaways:**

* P95/P99 TTFT represents the experience of your most frustrated users
* Three bottlenecks drive tail latency: prefill queuing, KV cache misses, and unpredictable queue depth
* More GPUs increase capacity but do **not** reduce tail latency—cache misses still occur with round-robin routing
* llm-d's prefix-cache-aware routing routes similar requests to the same instance, eliminating redundant prefill computation and reducing P95/P99 TTFT

**Next steps:**

Module 2 runs a short illustrative GuideLLM benchmark against this single GPU to observe queue saturation under high concurrency.
